...
with open(RELATIVE_CONFIG_PATH+USER_COLLECTION+'.csv', 'r') as user_fh:
    for user_row in user_fh:
        ...
        if user_row:
            (username, email, role, writeDevicesList, readDevicesList) = user_row.split(',')
        rwdevices = writeDevicesList.split('|')
        rdevices = readDevicesList.split('|')
        #if(role == 'admin'):
        #    user_data = {'username': username, 'email': email, 'role': role}
        #else:
        user_data = {'username': username, 'email': email, 'role': role, 'devices': {'rw':rwdevices, 'r':rdevices}}
